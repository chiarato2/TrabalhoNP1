...
//modelo Prioridade que simula os processos por prioridade
public class Prioridade extends JFrame{
	
	public Processo fila[];
	public int quant, soma = 0, soma1 = 0, prioridade;
	public double tempo_medio, t_resposta_medio;
	Container painel;
	JButton botao;
	TextField quantidade;
	JLabel l_quant, l_processo, l_tempoExecucao,l_espera, l_tempo_medio, l_resposta, l_resposta_media, l_prioridade;
	List processo, execucao, espera, resposta, prio;
	
	//tela onde irá aparecer as informações
	Prioridade(){
	...
	//método que calcula insere as informações no jframe
	private void calcular() {
		botao.addMouseListener( new MouseAdapter() {
            public void mouseClicked(MouseEvent evento) {
                   ...
                   l_prioridade = new JLabel("Prioridade");
                   
                   //considerando que entre os numeros {1, 2, 3, 4, 5, ...}, o numero 1 tem mais prioridade e 5 menos prioridade e assim por diante.
                   //quanto maior o número, menor a sua prioridade
                   for(int x = 0; x < quant; x++){
                	   int exec = Integer.parseInt(JOptionPane.showInputDialog("Tempo de execução do Processo " + x));
                	   fila[x] = new Processo("P" + x, exec);
                	   int prioridade = Integer.parseInt(JOptionPane.showInputDialog("Prioridade do processo " + x));
                	   fila[x].setPrioridade(prioridade);
                   }
                   ...
                   prio.setBounds(360,200,50,(quant * 20));
                   ...
                   l_prioridade.setBounds(360,180,70,25);
                   Resultado();
            }
		});
	}
	...
	//ordena o processo com maior prioridade no topo da lista
	private void ordenarPrioridade() {
		for(int x = 0; x < this.quant; x++){
			for(int y = 0; y <= x; y++){
				if(this.fila[x].getPrioridade() < this.fila[y].getPrioridade()){
	...
	private void Resultado() {
		for(int x = 0; x < this.quant; x++){
			...
			this.prio.add(""+this.fila[x].getPrioridade());
		}
